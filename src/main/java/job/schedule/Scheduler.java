...
public class Scheduler {
    ...
    */
    private final BlockingQueue<TimedTask> queue = new PriorityBlockingQueue<TimedTask>(CAPACITY,
            (s, t) -> {
                if (s.getJob().getJobConfigurations().getPriority().getPriority() > t.getJob().getJobConfigurations().getPriority().getPriority())
                    return 1;
                else if (s.getJob().getJobConfigurations().getPriority().getPriority() < t.getJob().getJobConfigurations().getPriority().getPriority())
                    return -1;
                return 0;
